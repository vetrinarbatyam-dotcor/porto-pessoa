"""FAROL scraper — Supercasa.pt."""
from .base import SourceScraper, RawListing, browser, polite_delay, parse_price, parse_area, parse_typology, infer_freguesia


class SupercasaScraper(SourceScraper):
    source = "supercasa"
    base_url = (
        "https://www.supercasa.pt/comprar/apartamentos/porto-porto"
        "?pmin={budget_min}&pmax={budget_max}"
    )
    ...
    def scrape(self, budget_min=150_000, budget_max=650_000):
        ...
        with browser() as ctx:
            ...
            for pagenum in range(1, self.max_pages + 1):
                purl = url + f"&pag={pagenum}" if pagenum > 1 else url
                try:
                    ...
                    break
                cards = page.query_selector_all("article, .property-item, [class*='listing']")
                if not cards:
                    self.log(f"no cards p{pagenum} — stop")
                    break
                for card in cards:
                    try:
                        a = card.query_selector("a[href*='/imovel/'], a[href*='/anuncio/'], a")
                        if not a:
                            continue
                        href = a.get_attribute("href") or ""
                        link = href if href.startswith("http") else "https://www.supercasa.pt" + href
